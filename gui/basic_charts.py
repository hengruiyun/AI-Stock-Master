...
class MarketChart(BaseChart):
    ...
    def _plot_sentiment_history(self, ax, history_data: List[Dict]):
        ...
        # 添加参考线
        ax.axhline(y=70, color=self.colors['success'], linestyle='--', alpha=0.5, label='乐观线')
        ax.axhline(y=50, color=self.colors['info'], linestyle='--', alpha=0.5, label='中性线')
        ax.axhline(y=30, color=self.colors['danger'], linestyle='--', alpha=0.5, label='悲观线')
    ...
    def plot_risk_analysis(self, risk_data: Dict[str, Any]):
        ...
        # 绘制雷达图
        ax.plot(angles, values, color=self.colors['danger'], linewidth=2, label='当前风险水平')
...
if __name__ == "__main__":
    ...
    notebook.add(trend_frame, text="趋势图")
    ...
    notebook.add(industry_frame, text="行业对比")
    ...
    notebook.add(market_frame, text="市场情绪")
