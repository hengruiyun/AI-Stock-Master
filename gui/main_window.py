...
class StockAnalyzerMainWindow:
        ...
        try:
            ...
            for icon_path in icon_paths:
                if os.path.exists(icon_path):
                    ...
                    print(f"{t_tools('window_icon_set_success')}: {icon_path}")
                    return
    ...
    def setup_menu(self):
        ...
        file_menu.add_command(label=t_gui("menu_exit"), command=self.root.quit, 
                             accelerator=t_gui("shortcut_alt_f4"))
        ...
        tools_menu.add_command(label=t_gui("menu_update_data"), command=self.update_data_files)
        ...
        menubar.add_cascade(label=t_gui("menu_help") + "(H)", menu=help_menu, underline=2)
        help_menu.add_command(label=t_gui("menu_user_guide"), command=self.open_user_guide)
        help_menu.add_command(label=t_gui("menu_about"), command=self.show_about)
...
{t_gui("result_calculation_time")}: 10-15{t_gui("trading_days")}
...
{t_gui("excellent")} {t_gui("quality_stocks_top5")} ({t_gui("sorted_by_rtsi")}):"""
        ...
        try:
            ...
            if hasattr(self.analysis_results, 'metadata'):
                ...
                market_state_translations = {
                    'healthy_optimism': t_gui("healthy_optimism"),
                    'cautious_optimism': t_gui("cautious_optimism"),
                    'euphoric': t_gui("euphoric"),
                    'optimistic': t_gui("optimistic"),
                    'neutral': t_gui("neutral"),
                    'pessimistic': t_gui("pessimistic"),
            ...
            if top_stocks:
                for i, stock_data in enumerate(top_stocks[:5], 1):
                    if isinstance(stock_data, tuple) and len(stock_data) >= 3:
                        ...
                        recommendation = t_gui('strongly_recommend') if rtsi_value > 70 else t_gui('moderate_attention') if rtsi_value > 50 else t_gui('cautious_watch')
            ...
            <tr>
                <td>{i}</td>
                <td>--</td>
                <td>{t_gui('data_processing')}</td>
                <td>--</td>
                <td>{t_gui('waiting_analysis')}</td>
            </tr>"""
            else:
                stock_recommendations_html = f"""
            <tr>
            ...
            if hasattr(self.analysis_results, 'industries') and self.analysis_results.industries:
                ...
                if top_industries:
                    ...
                    industry_analysis_html += f"<p><strong>{get_text('report', 'current_strongest_industry')}:</strong> {strongest_industry} ({get_text('report', 'irsi_index')}: {strongest_irsi:.2f})</p>"
    ...
    <div class="section">
        ...
        <p><small>{t_gui("ai_analysis_disclaimer")}</small></p>
            ...
            html_content = f"""
<!DOCTYPE html>
<html lang="{{"zh-CN" if get_current_language() == "zh_CN" else "en-US"}}">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    <div class="section">
        ...
        <div class="metric">{get_text('report', 'analysis_algorithm')}: <span class="highlight">RTSI + TMA + MSCI</span></div>
    ...
    def show_about(self):
        """显示关于窗口"""
        messagebox.showinfo(t_gui('about'), f"{t_gui('ai_stock_trend_analysis_system')} {VERSION}\n\n{t_gui('professional_stock_analysis_tool')}\n\n{t_gui('contact')}: {AUTHOR}")
    ...
    def show_github_info_window(self):
        ...
        # 设置窗口标题和内容
        github_window.title("GitHub Star - AI股票大师")
        title_text = "AI股票大师"
        star_button_text = t_gui("star")
                ...
                for stock_code, stock_name, score in top_stocks:
                    if stock_code in self.analysis_results.stocks:
                        ...
                        if isinstance(rtsi_data, dict):
                            from config.i18n import t_rtsi
                            rtsi_value = rtsi_data.get(t_rtsi('rtsi'), rtsi_data.get('RTSI', rtsi_data.get('rtsi', 0)))
                        else:
...
"""
        ...
        if data.get("market_data"):
            ...
            if is_english():
                # 市场状态判断（英文）
                if msci > 80:
                    market_state = t_gui("extremely_optimistic")
                elif msci > 65:
                    market_state = t_gui("healthy_optimistic")
                elif msci > 55:
                    market_state = t_gui("cautiously_optimistic")
                elif msci > 45:
                    market_state = t_gui("neutral")
                elif msci > 35:
                    market_state = t_gui("mildly_pessimistic")
                elif msci > 25:
                    market_state = t_gui("significantly_pessimistic")
                else:
                    market_state = t_gui("panic_selling")
                
                # 趋势描述
                if trend > 10:
                    trend_desc = t_gui("strong_uptrend")
                elif trend > 3:
                    trend_desc = t_gui("moderate_uptrend")
                elif abs(trend) <= 3:
                    trend_desc = t_gui("sideways")
                elif trend > -10:
                    trend_desc = t_gui("moderate_downtrend")
                else:
                    trend_desc = t_gui("sharp_decline")
                
                # 波动率描述
                if volatility > 30:
                    vol_desc = t_gui("extremely_high_volatility")
                elif volatility > 20:
                    vol_desc = t_gui("high_volatility")
                elif volatility > 10:
                    vol_desc = t_gui("medium_volatility")
                elif volatility > 5:
                    vol_desc = t_gui("low_volatility")
                else:
                    vol_desc = t_gui("very_low_volatility")
        ...
        if data.get("macro_indicators"):
            ...
            if current_is_english:
                ...
                prompt += "## 🌍 宏观经济环境\n"
                prompt += f"- 基准利率: {macro.get('interest_rate', 0):.1f}% (货币政策方向)\n"
                prompt += f"- 通胀水平: {macro.get('inflation_rate', 0):.1f}% (价格稳定性)\n"
                prompt += f"- GDP增长: {macro.get('gdp_growth', 0):.1f}% (经济增长动能)\n"
        ...
        if data.get("news_sentiment"):
            ...
            if sentiment_score > 0.3:
                sentiment_desc = t_gui("positive_optimistic")
            elif sentiment_score > -0.3:
                sentiment_desc = t_gui("neutral_balanced")
            else:
                sentiment_desc = t_gui("negative_pessimistic")
            ...
            if current_is_english:
                ...
                prompt += "## 📰 市场情绪分析\n"
                prompt += f"- 整体情绪倾向: {sentiment_score:+.2f} ({sentiment_desc})\n"
                prompt += f"- 正面新闻比例: {news.get('positive_ratio', 0):.1%}\n"
                prompt += f"- 负面新闻比例: {news.get('negative_ratio', 0):.1%}\n"
                prompt += f"- 中性新闻比例: {news.get('neutral_ratio', 0):.1%}\n"
                prompt += f"- 新闻活跃度: {news.get('news_volume', 0)} 篇/日\n\n"
        
        # 行业数据
        if data.get("industry_data"):
            if current_is_english:
                prompt += "## Technical Momentum Analysis (TMA)\n"
            else:
                prompt += "## 技术指标分析 (TMA)\n"
            ...
            for i, (industry, info) in enumerate(sorted_industries[:5]):
                ...
                if irsi > 70:
                    ...
                    strength = t_gui("very_weak")
                
                rank_text = "#1" if i == 0 else "#2" if i == 1 else "#3" if i == 2 else "#4" if i == 3 else "#5"
                if current_is_english:
                    prompt += f"{rank_text} {industry}: TMA={irsi:.2f} ({strength}), {stock_count} stocks, Avg Daily Volume {avg_volume/10000:.1f}M\n"
                else:
                    prompt += f"{rank_text} {industry}: TMA={irsi:.2f} ({strength}), {stock_count} 只股票, 日均成交量 {avg_volume/10000:.1f}万\n"
            prompt += "\n"
        ...
        if data.get("stock_data"):
            ...
            for i, (formatted_code, info) in enumerate(sorted_stocks[:10]):
                ...
                # 个股强度评级
                if rtsi > 80:
                    rating = t_gui("strongly_recommend")
                elif rtsi > 65:
                    rating = t_gui("buy")
                elif rtsi > 50:
                    rating = t_gui("moderate_attention")
                elif rtsi > 35:
                    rating = t_gui("cautious_watch")
                else:
                    rating = t_gui("avoid")
                
                rank_text = f"#{i+1}"
                # 只有当行业信息存在且有意义时才显示
                if industry and industry not in ['N/A', 'Unknown', '未知', '未分类', '', 'NULL']:
                    if current_is_english:
                        prompt += f"{rank_text} {formatted_code} ({stock_name}): RTSI={rtsi:.2f}, Industry: {industry}, Rating: {rating}\n"
                    else:
                        prompt += f"{rank_text} {formatted_code} ({stock_name}): RTSI={rtsi:.2f}, 行业: {industry}, 评级: {rating}\n"
                else:
                    if current_is_english:
                        prompt += f"{rank_text} {formatted_code} ({stock_name}): RTSI={rtsi:.2f}, Rating: {rating}\n"
                    else:
                        prompt += f"{rank_text} {formatted_code} ({stock_name}): RTSI={rtsi:.2f}, 评级: {rating}\n"
            prompt += "\n"
        
        # 分析要求 - 更丰富的提示内容
        if current_is_english:
            prompt += """## Comprehensive Professional Analysis Framework

As a senior quantitative analyst with deep market expertise, please provide comprehensive analysis incorporating multiple timeframes and risk perspectives:

### I. Market Cycle & Sentiment Analysis
1. **Market Phase Identification**: Based on MSCI indicators, determine current market cycle stage (Accumulation/Markup/Distribution/Decline)
2. **Sentiment Extremes Detection**: Analyze market fear/greed levels and identify potential reversal signals
3. **Liquidity Flow Analysis**: Assess capital inflow/outflow patterns through volume amplification metrics
4. **Policy Impact Assessment**: Evaluate regulatory and monetary policy effects on market sentiment

### II. Advanced Sector Rotation Strategy  
5. **Sector Leadership Analysis**: Based on TMA rankings, identify sector rotation patterns and leadership changes
6. **Cross-Sector Momentum**: Compare relative strength across different economic sectors
7. **Thematic Investment Opportunities**: Identify emerging themes and structural growth sectors
8. **Dynamic Sector Allocation**: Provide adaptive sector weight recommendations based on cycle positioning

### III. Sophisticated Stock Selection & Timing
9. **Multi-Factor Stock Screening**: Integrate RTSI ratings with fundamental and technical factors
10. **Precision Entry/Exit Points**: Utilize historical RTSI patterns to identify optimal timing windows
11. **Risk-Adjusted Analysis**: Evaluate potential returns relative to sector and market benchmarks
12. **Catalyst-Driven Opportunities**: Identify stocks with upcoming catalysts or events

### IV. Dynamic Risk Management Framework
13. **Systematic Risk Monitoring**: Real-time assessment of market stress indicators and volatility regimes
14. **Portfolio Correlation Analysis**: Track concentration risks and correlation breakdowns
15. **Tail Risk Protection**: Implement strategies for managing extreme market events
16. **Adaptive Position Sizing**: Dynamic position management based on market conditions

### V. Actionable Investment Roadmap
17. **Short-term Tactical Moves** (1-4 weeks): Specific recommendations with entry/exit points
18. **Medium-term Strategic Positioning** (1-6 months): Sector rotation and thematic plays
19. **Long-term Wealth Building** (6+ months): Structural growth opportunities
20. **Crisis Management Protocols**: Clear guidelines for market stress scenarios

**Enhanced Output Requirements**: Provide comprehensive, actionable insights with specific recommendations, confidence levels, and risk assessments. Content should be 1000-1200 words, emphasizing depth, practicality, and professional investment logic. Include specific price targets and timeframes where applicable."""
        else:
            prompt += """## 全面专业分析框架

作为资深量化分析师和投资策略专家，请结合多时间维度和风险视角，提供全方位的投资分析：

### 一、市场周期与情绪分析
1. **市场阶段识别**：基于MSCI指标，判断当前市场周期阶段（蓄势/上升/分发/下跌）
2. **情绪极端检测**：分析市场恐慌/贪婪水平，识别潜在反转信号
3. **资金流向分析**：通过成交量放大指标评估资金流入/流出模式
4. **政策影响评估**：评估监管和货币政策对市场情绪的影响

### 二、高级板块轮动策略
5. **板块领导分析**：基于TMA排名，识别板块轮动模式和领导力变化
6. **跨板块动量对比**：比较不同经济板块间的相对强度表现
7. **主题投资机会**：识别新兴主题和结构性增长板块
8. **动态板块配置**：基于周期定位提供自适应板块权重建议

### 三、精深个股筛选与择时
9. **多因子股票筛选**：整合RTSI评级与基本面、技术面等多重因子
10. **精准进出场点**：利用历史RTSI模式识别最佳时机窗口
11. **风险调整分析**：评估相对板块和市场基准的风险调整收益
12. **催化剂驱动机会**：识别具有即将到来催化剂或重大事件的股票

### 四、动态风险管理框架
13. **系统性风险监控**：实时评估市场压力指标和波动率环境变化
14. **组合相关性分析**：跟踪持仓集中度风险和相关性失效情况
15. **尾部风险保护**：针对极端市场事件制定风险管理策略
16. **自适应仓位管理**：基于市场环境和个股强度的动态仓位调整

### 五、可操作投资路线图
17. **短期战术操作**（1-4周）：具体交易建议和进出场点位
18. **中期战略布局**（1-6个月）：板块轮动机会和主题投资策略
19. **长期财富积累**（6个月以上）：结构性增长机会和价值投资
20. **危机应对方案**：市场压力情景下的具体操作指导

**增强输出要求**：请提供全面、可操作的投资洞察，包含具体建议、置信度评估和风险提示。内容应控制在1000-1200字，确保分析的专业深度和实用性。对重要建议请提供具体的价格目标和时间框架。"""
    ...
    def _evaluate_ai_reliability(self, ai_response: str, analysis_data: dict) -> dict:
        ...
        try:
            ...
            if risk_mentions < 2:
                score -= 0.5
                if is_english():
                    explanations.append(t_gui("insufficient_risk_warnings"))
                else:
            ...
            if analysis_data.get('news_sentiment'):
                ...
                if abs(sentiment_score) > 1:
                    score -= 0.5
                    if is_english():
                        explanations.append(t_gui("news_sentiment_out_of_range"))
                    else:
...
""")
            ...
            messagebox.showerror(t_gui('error'), f"{t_gui('load_stock_list_failed')}: {str(e)}")
    ...
    def analyze_selected_stock(self):
        ...
        if search_input:
            ...
            selected = self.stock_combo.get()
            if not selected:
                messagebox.showwarning(t_gui('tip'), t_gui('input_stock_code_or_select'))
                return
        ...
        try:
            ...
            if hasattr(self.analysis_results, 'stocks') and stock_code in self.analysis_results.stocks:
                ...
                # RTSI分析数据 - 修复：使用正确的国际化键名
                rtsi_data = stock_data.get('rtsi', {})
                if isinstance(rtsi_data, dict):
                    # RTSI计算结果使用国际化键名，需要正确获取
                    from config.i18n import t_rtsi
                    rtsi_value = rtsi_data.get(t_rtsi('rtsi'), rtsi_data.get('RTSI', rtsi_data.get('rtsi', 0)))
                    confidence = rtsi_data.get(t_rtsi('confidence'), rtsi_data.get('置信度', rtsi_data.get('confidence', 0)))
                    slope = rtsi_data.get(t_rtsi('slope'), rtsi_data.get('斜率', rtsi_data.get('slope', 0)))
                ...
                # 显示找不到股票的错误信息
                messagebox.showerror(t_gui('stock_not_found'), 
                                   f"{t_gui('stock_code_not_found_msg')} '{stock_code}' 的数据\n\n"
                                   f"{t_gui('possible_reasons')}：\n"
                                   f"1. {t_gui('stock_code_not_exist')}\n"
                                   f"2. {t_gui('stock_not_in_dataset')}\n"
                                   f"3. 请检查股票代码格式\n\n"
                                   f"建议：\n"
                                   f"- 检查股票代码是否正确\n"
                                   f"- 从下拉列表中选择已有股票\n"
            ...
            messagebox.showerror(t_gui('analysis_error'), f"{t_gui('stock_analysis_failed')}:\n{str(e)}")
...
"""
    ...
    def generate_and_plot_realistic_data(self, stock_code, stock_data, source_type="RTSI的智能模拟"):
        ...
        try:
            ...
            rtsi_data = stock_data.get('rtsi', {})
            if isinstance(rtsi_data, dict):
                current_rtsi = rtsi_data.get('RTSI', rtsi_data.get('rtsi', 50))
            else:
    ...
    def generate_detailed_analysis(self, stock_code, stock_name, stock_data, rtsi_data):
        """生成详细分析报告 - 完整版本"""
        try:
            rtsi_value = rtsi_data.get('RTSI', rtsi_data.get('rtsi', 0)) if isinstance(rtsi_data, dict) else rtsi_data
...
• {t_gui('industry_strong')}: RSI ≥ 70, {t_gui('significantly_outperform_market')}
• {t_gui('industry_neutral_strong')}: 60 ≤ RSI < 70, {t_gui('slightly_outperform')}
• {t_gui('industry_neutral')}: 40 ≤ RSI < 60, {t_gui('sync_with_market')}
• {t_gui('industry_neutral_weak')}: 30 ≤ RSI < 40, {t_gui('slightly_underperform')}
• {t_gui('industry_weak')}: RSI < 30, {t_gui('significantly_underperform_market')}

{t_gui('tips')} {t_gui('usage_suggestions')}:
1. {t_gui('focus_strong_industries_above_70')}
2. {t_gui('avoid_weak_industries_below_30')}
...
"""
    ...
    def get_strength_level(self, irsi_value):
        """获取强度等级"""
        if irsi_value >= 70:
            return t_gui("hot_strong")
        elif irsi_value >= 60:
            return t_gui("rising_moderately_strong")
        elif irsi_value >= 40:
            return t_gui("neutral")
        elif irsi_value >= 30:
            return t_gui("declining_moderately_weak")
        else:
            return t_gui("cold_weak")
    
    def get_trend_analysis(self, irsi_value):
        """获取趋势分析"""
        if irsi_value >= 70:
            return t_gui("clear_uptrend_capital_inflow")
        elif irsi_value >= 50:
            return t_gui("moderate_rise_outperforming")
        elif irsi_value >= 30:
            return t_gui("consolidation_waiting_direction")
        else:
            return t_gui("downtrend_capital_outflow")
    
    def get_investment_value(self, irsi_value):
        """获取投资价值"""
        if irsi_value >= 70:
            return t_gui('high_value')
        elif irsi_value >= 60:
            return t_gui('higher_value')
        elif irsi_value >= 40:
            return t_gui('medium_value')
        elif irsi_value >= 30:
            return t_gui('lower_value')
        else:
            return t_gui('low_value')
    
    def get_risk_level(self, irsi_value):
        """获取风险等级"""
        if irsi_value >= 70:
            return t_gui('low_risk')
        elif irsi_value >= 50:
            return t_gui('medium_low_risk')
        elif irsi_value >= 30:
            return t_gui('medium_high_risk')
        else:
            return t_gui('high_risk')
    
    def get_investment_advice(self, industry_name, irsi_value):
        """获取投资建议"""
        if irsi_value >= 70:
            return f"• {t_gui('actively_allocate')} {industry_name} {t_gui('industry_leading_stocks')}\n• {t_gui('can_increase_position_ratio')}\n• {t_gui('focus_sector_rotation_opportunities')}"
        elif irsi_value >= 60:
            return f"• {t_gui('can_moderately_allocate')} {industry_name} {t_gui('industry')}\n• {t_gui('recommend_select_higher_rtsi')}\n• {t_gui('control_position_manage_risks')}"
        elif irsi_value >= 40:
            return f"• {industry_name} {t_gui('industry_neutral_performance')}\n• {t_gui('can_balance_allocation')}\n• {t_gui('monitor_fundamentals_closely')}"
        elif irsi_value >= 30:
            return f"• {industry_name} {t_gui('industry_weak_performance')}\n• {t_gui('recommend_reduce_or_avoid')}\n• {t_gui('wait_stabilization_signal')}"
        else:
            return f"• {industry_name} {t_gui('industry_poor_performance')}\n• {t_gui('recommend_temporary_avoidance')}\n• {t_gui('wait_turning_point')}"
    
    def get_risk_warning(self, irsi_value):
        """获取风险提示"""
        if irsi_value >= 70:
            return t_gui("watch_pullback_risks_high_levels")
        elif irsi_value >= 50:
            return t_gui("remain_cautiously_optimistic")
        elif irsi_value >= 30:
            return t_gui("control_position_risks")
        else:
            return t_gui("high_risk_control_losses")
    ...
    def export_industry_data(self):
        ...
        try:
            ...
            messagebox.showinfo(t_gui('success'), f"{t_gui('industry_data_exported')}:\n{filename}")
            
        except Exception as e:
            messagebox.showerror(t_gui('error'), f"{t_gui('export_failed')}:\n{str(e)}")
...
"""
        ...
        messagebox.showinfo(t_gui('msci_algorithm_details'), detail_info)
...
• 关注系统性风险
"""
                messagebox.showwarning(t_gui('market_alert'), alert_msg)
...
• 控制单次建仓规模
"""
                messagebox.showinfo(t_gui('investment_opportunity'), alert_msg)
...
"""
                messagebox.showinfo(t_gui('market_status'), alert_msg)
                
        except Exception as e:
            messagebox.showerror(t_gui('error'), f"{t_gui('get_market_alert_failed')}:\n{str(e)}")
    ...
    def open_ai_model_settings(self):
        ...
        try:
            ...
            if os.path.exists(run_settings_no_console_path):
                ...
                messagebox.showerror(t_gui('error'), f"{t_gui('ai_model_settings_file_not_found')}:\n{run_settings_no_console_path}\n或\n{run_settings_path}")
                
        except Exception as e:
            messagebox.showerror(t_gui('error'), f"{t_gui('start_ai_model_settings_failed')}:\n{str(e)}")
