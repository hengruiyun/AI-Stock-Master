    ...
    def load_stock_data(self, stocks_data: Dict[str, Any]):
        ...
        for stock_code, stock_info in stocks_data.items():
            ...
            values = (
                ...
                trend_mapping.get(rtsi_data.get('trend', 'sideways'), t_gui('unknown')),
...
{t_gui('rtsi_analysis_result')}:"""
        ...
        rtsi_data = stock_info.get('rtsi', {})
        info_text += f"""
• {t_gui('rtsi_index')}: {rtsi_data.get('rtsi', 0):.2f}
• {t_gui('trend_direction')}: {rtsi_data.get('trend', t_gui('unknown'))}
• {t_gui('data_reliability')}: {rtsi_data.get('confidence', 0):.3f}
• {t_gui('slope')}: {rtsi_data.get('slope', 0):.4f}
• {t_gui('r_squared')}: {rtsi_data.get('r_squared', 0):.3f}

{t_gui('recent_rating')}:
• {t_gui('latest_rating')}: {rtsi_data.get('recent_score', t_gui('unknown'))}
• {t_gui('five_day_change')}: {rtsi_data.get('score_change_5d', 'N/A')}
"""
