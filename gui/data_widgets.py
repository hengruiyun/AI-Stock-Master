...
# 导入国际化配置
try:
    from config.i18n import t_gui, t_common, get_current_language
except ImportError:
    # 回退函数
    def t_gui(key, default=None):
...
class StockListWidget(ttk.Treeview):
    ...
    def __init__(self, parent, **kwargs):
        ...
        # 设置列标题
        self.heading('#1', text=t_gui('column_code'), anchor=tk.W)
        self.heading('#2', text=t_gui('column_name'), anchor=tk.W)
        self.heading('#3', text=t_gui('column_industry'), anchor=tk.W)
        self.heading('#4', text='RTSI', anchor=tk.E)
        self.heading('#5', text=t_gui('column_trend'), anchor=tk.CENTER)
        self.heading('#6', text=t_gui('column_confidence'), anchor=tk.E)
    ...
    def load_stock_data(self, stocks_data: Dict[str, Any]):
        ...
        for stock_code, stock_info in stocks_data.items():
            ...
            # 趋势方向中文映射
            trend_mapping = {
                'strong_up': t_gui('trend_strong_up'),
                'weak_up': t_gui('trend_weak_up'), 
                'sideways': t_gui('trend_sideways'),
                'weak_down': t_gui('trend_weak_down'),
                'strong_down': t_gui('trend_strong_down')
            }
            
            # 修复：使用正确的国际化键名获取RTSI数据
            from config.i18n import t_rtsi
            rtsi_value = rtsi_data.get(t_rtsi('rtsi'), rtsi_data.get('RTSI', rtsi_data.get('rtsi', 0)))
            trend_value = rtsi_data.get(t_rtsi('trend'), rtsi_data.get('趋势', rtsi_data.get('trend', 'sideways')))
            confidence_value = rtsi_data.get(t_rtsi('confidence'), rtsi_data.get('置信度', rtsi_data.get('confidence', 0)))
            
            values = (
                stock_code,
                stock_info.get('name', t_gui('unknown')),
                stock_info.get('industry', t_gui('uncategorized')),
                f"{rtsi_value:.2f}",
                trend_mapping.get(trend_value, t_gui('unknown')),
...
class AnalysisResultWidget(tk.Text):
    ...
    def display_analysis_result(self, analysis_data: Dict[str, Any]):
        ...
        # 标题
        self.append_text(f"{t_gui('analysis_result_title')}\n\n", 'title')
        
        # 数据概览
        summary = analysis_data.get('summary', {})
        self.append_text(f"{t_gui('data_overview')}\n", 'subtitle')
        self.append_text(f"• {t_gui('total_stocks')}: ")
        self.append_text(f"{summary.get('total_stocks', 0)}", 'number')
        self.append_text(f" {t_gui('stocks_unit')}\n")
        
        self.append_text(f"• {t_gui('industry_categories')}: ")
        self.append_text(f"{summary.get('total_industries', 0)}", 'number')
        self.append_text(f" {t_gui('categories_unit')}\n")
        
        self.append_text(f"• {t_gui('rating_coverage')}: ")
        self.append_text(f"{summary.get('rating_coverage', 0):.1f}%", 'number')
        self.append_text("\n\n")
        
        # 市场情绪
        market = analysis_data.get('market_sentiment', {})
        self.append_text(f"{t_gui('market_sentiment_analysis')}\n", 'subtitle')
        self.append_text(f"• {t_gui('msci_index')}: ")
        self.append_text(f"{market.get('current_msci', 0):.2f}", 'number')
        self.append_text("\n")
        
        state = market.get('market_state', t_gui('unknown'))
        state_color = 'success' if state in ['optimistic', 'euphoric'] else 'warning' if state == 'neutral' else 'error'
        self.append_text(f"• {t_gui('market_state')}: ")
...
class RankingWidget(ttk.Treeview):
    ...
    def __init__(self, parent, ranking_type="stock", **kwargs):
        ...
        if ranking_type == "stock":
            ...
            self.column_headers = [t_gui('column_rank'), t_gui('column_code'), t_gui('column_name'), 'RTSI', t_gui('column_trend')]
        elif ranking_type == "industry":
            columns = ('rank', 'industry', 'irsi', 'status', 'stocks')
            self.column_headers = [t_gui('column_rank'), t_gui('column_industry'), 'IRSI', t_gui('column_status'), t_gui('column_stock_count')]
        else:
            columns = ('rank', 'item', 'value', 'status')
            self.column_headers = [t_gui('column_rank'), t_gui('column_item'), t_gui('column_value'), t_gui('column_status')]
    ...
    def load_industry_ranking(self, industries_data: List[Dict[str, Any]]):
        ...
        # 状态映射
        status_mapping = {
            'strong_outperform': t_gui('status_strong_outperform'),
            'weak_outperform': t_gui('status_weak_outperform'),
            'neutral': t_gui('status_neutral'),
            'weak_underperform': t_gui('status_weak_underperform'),
            'strong_underperform': t_gui('status_strong_underperform')
        }
        ...
        for i, industry in enumerate(sorted_industries, 1):
            values = (
                i,
                industry.get('name', t_gui('unknown')),
                f"{industry.get('irsi', 0):.2f}",
                status_mapping.get(industry.get('status', 'neutral'), t_gui('unknown')),
...
class StockDetailWindow:
    ...
    def __init__(self, parent, stock_code, stock_name, stock_info):
        ...
        self.window.title(f"{t_gui('stock_detail_title')} - {stock_name} ({stock_code})")
    ...
    def setup_content(self, stock_code, stock_name, stock_info):
        ...
        # 基本信息
        info_text = f"""{t_gui('basic_info')}:
• {t_gui('stock_code')}: {stock_code}
• {t_gui('stock_name')}: {stock_name}
• {t_gui('industry')}: {stock_info.get('industry', t_gui('uncategorized'))}

{t_gui('rtsi_analysis_result')}:"""
        ...
        from config.i18n import t_rtsi
        rtsi_value = rtsi_data.get(t_rtsi('rtsi'), rtsi_data.get('RTSI', rtsi_data.get('rtsi', 0)))
        trend_value = rtsi_data.get(t_rtsi('trend'), rtsi_data.get('趋势', rtsi_data.get('trend', t_gui('unknown'))))
        confidence_value = rtsi_data.get(t_rtsi('confidence'), rtsi_data.get('置信度', rtsi_data.get('confidence', 0)))
        slope_value = rtsi_data.get(t_rtsi('slope'), rtsi_data.get('斜率', rtsi_data.get('slope', 0)))
        r_squared_value = rtsi_data.get(t_rtsi('r_squared'), rtsi_data.get('R²值', rtsi_data.get('r_squared', 0)))
        recent_score_value = rtsi_data.get(t_rtsi('recent_score'), rtsi_data.get('recent_score', t_gui('unknown')))
        score_change_5d_value = rtsi_data.get(t_rtsi('score_change_5d'), rtsi_data.get('score_change_5d', 'N/A'))
        
        info_text += f"""
• {t_gui('rtsi_index')}: {rtsi_value:.2f}
• {t_gui('trend_direction')}: {trend_value}
• {t_gui('data_reliability')}: {confidence_value:.3f}
• {t_gui('slope')}: {slope_value:.4f}
• {t_gui('r_squared')}: {r_squared_value:.3f}

{t_gui('recent_rating')}:
• {t_gui('latest_rating')}: {recent_score_value}
• {t_gui('five_day_change')}: {score_change_5d_value}
"""
        ...
        # 关闭按钮
        close_btn = tk.Button(content_frame, text=t_gui('close'), 
