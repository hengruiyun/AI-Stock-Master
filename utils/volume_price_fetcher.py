...
try:
    ...
    try:
        # 回退到当前目录导入
        from lj_data_reader import StockSearchTool
    except ImportError:
        print("警告: 无法导入lj数据读取模块，量价数据功能将不可用")
...
class VolumePriceFetcher:
    ...
    def __init__(self, verbose: bool = False):
        ...
        self.data_file_mapping = {
            'cn': 'cn-lj.dat.gz' if os.path.exists('cn-lj.dat.gz') else 'cn-lj.dat',
            'hk': 'hk-lj.dat.gz' if os.path.exists('hk-lj.dat.gz') else 'hk-lj.dat',
            'us': 'us-lj.dat.gz' if os.path.exists('us-lj.dat.gz') else 'us-lj.dat'
        }
