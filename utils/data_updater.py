#!/usr/bin/env python3
# -*- coding: utf-8 -*-
from config.gui_i18n import t_gui, get_system_language
is_english = lambda: get_system_language() == 'en'
# t_tools 功能由 t_gui 替代
t_tools = t_gui
"""
AI股票大师 - 数据自动更新模块
...
class DataUpdater:
    ...
    def __init__(self, parent=None):
        ...
        self.files_to_download = [
            ...
            ###
            #{
            #    'url': 'https://gh-proxy.com/https://github.com/hengruiyun/AI-Stock-Master/raw/refs/heads/main/cn-lj.dat',
            #    'filename': 'cn-lj.dat',
            #    'description': '中国市场量价数据dat'
            #},
            #{
            #    'url': 'https://gh-proxy.com/https://github.com/hengruiyun/AI-Stock-Master/raw/refs/heads/main/hk-lj.dat',
            #    'filename': 'hk-lj.dat',
            #    'description': '香港市场量价数据dat'
            #},
            #{
            #    'url': 'https://gh-proxy.com/https://github.com/hengruiyun/AI-Stock-Master/raw/refs/heads/main/us-lj.dat',
            #    'filename': 'us-lj.dat',
            #    'description': '美国市场量价数据dat'
            #}
            ###
        ]
    ...
    def _auto_fix_translations(self):
        """自动修复翻译结构问题"""
        try:
            from config import i18n
            if 'zh_CN' in i18n.TRANSLATIONS:
                zh_cn = i18n.TRANSLATIONS['zh_CN']
                # 如果tools在gui中，提取到顶级
                if 'gui' in zh_cn and 'tools' in zh_cn['gui'] and 'tools' not in zh_cn:
                    tools_section = zh_cn['gui'].pop('tools')
                    zh_cn['tools'] = tools_section
                # 如果utils在gui中，也提取到顶级
                if 'gui' in zh_cn and 'utils' in zh_cn['gui'] and 'utils' not in zh_cn:
                    utils_section = zh_cn['gui'].pop('utils')
                    zh_cn['utils'] = utils_section
        except Exception:
            pass  # 静默处理错误，避免影响正常功能
