...
class ChartStyleManager:
    ...
    def apply_theme(self, theme_name: str = 'professional'):
        """应用主题"""
...
class InteractiveTrendChart:
    ...
    def _create_plotly_trend(self, stock_data: Dict[str, Any], 
        ...
        fig.update_yaxes(title_text="RTSI值", row=1, col=1)
        fig.update_xaxes(title_text="评级", row=1, col=2)
        fig.update_yaxes(title_text="百分比", row=1, col=2)
        fig.update_xaxes(title_text="排名", row=2, col=1)
        fig.update_yaxes(title_text="RTSI值", row=2, col=1)
