...
class StockDataSet:
    ...
    def __init__(self, data_source, file_path: str = ""):
        ...
        if isinstance(data_source, str):
            # 根据文件扩展名选择合适的加载器
            if data_source.endswith('.json.gz'):
                from .compressed_json_loader import CompressedJSONLoader
                loader = CompressedJSONLoader(data_source)
            else:
    ...
    def _detect_market_type(self) -> str:
        ...
        if self.file_path:
            ...
            if file_name.startswith('us'):
                ...
                return 'cn'
            elif 'us' in file_name or 'america' in file_name:
                return 'us'
            elif 'hk' in file_name or 'hongkong' in file_name:
                return 'hk'
            elif 'cn' in file_name or 'china' in file_name:
                return 'cn'
        
        # 从股票代码格式判断
        if '股票代码' in self.data.columns and len(self.data) > 0:
    ...
    def get_stock_ratings(self, stock_code: str, use_interpolation: bool = True) -> pd.Series:
        ...
        """
        stock_code = self._normalize_stock_code(stock_code)  # 标准化代码格式
    ...
    def filter_stocks_by_codes(self, codes: List[str]) -> 'StockDataSet':
        ...
        返回:
            StockDataSet: 筛选后的数据集
        """
        codes = [self._normalize_stock_code(code) for code in codes]  # 标准化格式
