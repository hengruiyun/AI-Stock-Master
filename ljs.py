...
class StockSearchTool:
    ...
    def _filter_by_days(self, stock_info: Dict[str, Any], days: int = None) -> Dict[str, Any]:
        ...
        if days < len(sorted_dates):
            ...
            filtered_trade_data = {date: trade_data[date] for date in recent_dates}
        ...
        if output_format == "json":
            ...
            for key, stock_data in results.items():
                ...
                if trade_data:
                    ...
                    output.append("")
                    output.append(f"{'日期':<12} {'收盘价':<10} {'成交金额':<15}")
                    output.append("-" * 40)
                    
                    # 按日期排序显示
                    for date in sorted(trade_data.keys()):
                        price = trade_data[date].get('收盘价', 0)
                        volume = trade_data[date].get('成交金额', 0)
                        output.append(f"{date:<12} {price:<10.2f} {volume:<15,.0f}")
                else:
                    output.append("无交易数据")
            ...
            # CSV格式输出
            csv_lines = ["市场,股票代码,股票名称,交易日期,收盘价,成交金额"]
            ...
            for key, stock_data in results.items():
                ...
                for date in sorted(trade_data.keys()):
                    price = trade_data[date].get('收盘价', 0)
                    volume = trade_data[date].get('成交金额', 0)
                    csv_lines.append(f"{market},{code},{name},{date},{price},{volume}")
