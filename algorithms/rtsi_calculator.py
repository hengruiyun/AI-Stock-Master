...
class RTSICalculator:
    ...
    def calculate(self, stock_ratings: pd.Series, stock_code: str = None) -> Dict[str, Union[float, str, int, None]]:
        """
        计算单只股票的RTSI指数
        ...
        # 执行计算
        result = calculate_rating_trend_strength_index(stock_ratings)
    ...
    def batch_calculate_optimized(self, stock_data: pd.DataFrame, parallel: bool = False) -> Dict[str, Dict]:
        """
        优化的批量计算RTSI指数
        ...
        if parallel and len(stock_data) > 1000:
            return self._parallel_batch_calculate(stock_data)
        else:
            return batch_calculate_rtsi(stock_data)
    
    def _parallel_batch_calculate(self, stock_data: pd.DataFrame) -> Dict[str, Dict]:
        """
        并行批量计算（预留接口）
        """
        # 当前使用标准批量计算，后续可集成multiprocessing
        print("信息 大数据集检测，使用优化批量处理...")
        return batch_calculate_rtsi(stock_data)
    ...
    def batch_calculate(self, stock_data: pd.DataFrame) -> Dict[str, Dict]:
        ...
        返回:
            dict: 批量计算结果
        """
        return self.batch_calculate_optimized(stock_data, parallel=False)
