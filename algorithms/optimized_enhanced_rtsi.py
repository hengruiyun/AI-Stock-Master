...
try:
    ...
    print(f"{t_gui('导入依赖失败') if 't_gui' in locals() else '导入依赖失败'}: {e}")
    def t_rtsi(key): return key
    def t_gui(key, **kwargs): return key
...
class OptimizedEnhancedRTSI:
    ...
    def __init__(self, 
        ...
        print(f"🚀 {self.algorithm_name}{t_gui('计算器初始化完成')}")
        print(f"📊 {t_gui('配置参数: RTSI阈值')}={self.rtsi_threshold}, {t_gui('波动性阈值')}={self.volatility_threshold}")
        print(f"🎯 {t_gui('AI增强')}={self.use_ai_enhancement}, {t_gui('多维度')}={self.use_multi_dimensional}, {t_gui('时间窗口')}={self.time_window}{t_gui('天')}")
